(() => {
  ...
  const searchable = new Map(entries.map(e=>[e.id,norm([e.title,e.ru||'',e.code,e.sourceCode||'',e.aliases,...e.parents].join(' '))]));
  ...
  function updateHash() {
    ...
    if($('reader').open){if(state.printed!==null)p.set('page',state.printed);else if(state.pdf!==null)p.set('pdf',state.pdf);if(state.entry)p.set('section',state.entry.id);}
  ...
  function openPage(printed,pdf=null,entry=null,opener=null){
    ...
    if(printed===null && (!Number.isInteger(pdf)||pdf<1||pdf>469))return;
  ...
  function showScan(){
    ...
    $('page-position').textContent=state.printed!==null?'Книга · с. '+state.printed:'PDF · '+state.pdf+' / 469';
    $('scan-caption').textContent=state.pdf!==null?`Страница файла ${state.pdf} из 469${state.printed!==null?' · печатная '+state.printed:''}`:'Пропуск в исходном файле';
    $('prev-page').disabled=state.pdf===1;$('next-page').disabled=state.pdf===469;
    ...
    image.src=`scans/${String(state.pdf).padStart(3,'0')}.webp`;
  }
  function step(direction){
    ...
    if(target>=1&&target<=469)openPage(reversePages.get(target)??null,target,null);
  }
  ...
  function readHash(){
    ...
    const page=p.has('page')?Number(p.get('page')):null;const pdf=p.has('pdf')?Number(p.get('pdf')):null;
  ...
  for(const p of catalog.parts){
    ...
    button.addEventListener('click',()=>{state.part=state.part===p.id?'':p.id;state.topic='';state.limit=60;render();});$('parts').append(button);
  }
  ...
  $('topic').addEventListener('change',()=>{state.topic=$('topic').value;if(state.topic)state.part=catalog.topics.find(t=>t.id===state.topic).part;state.limit=60;render();});
  ...
  $('book-stats').textContent=`5 частей · ${entries.length} пунктов оглавления · 469 сканов`;
  readHash();
})();
